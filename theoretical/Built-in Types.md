...
- 2바이트(16비트): 시스템 내부적으로 사용되기도 하지만, 권장되지 않는다. (`wchar_t`, `char16_t`)
- 4바이트(32비트): 거의 사용되지 않는다. (`char32_t`)

UTF-16 방식은 호환성이나 이식성 등의 이유로 권장되지 않는다.
- 웹에서 거의 사용되지 않는다. (ASCII와 호환되지 않음)
- 유닉스 계열에서는 거의 사용되지 않는다.
- MS에서도 윈도우 애플리케이션에 UTF-8을 권장한다.
#### char
